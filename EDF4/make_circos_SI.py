...
for sector in circos.sectors:
    ...
    for count,acid in enumerate(Acids):
        if acid in s_condition.keys() and acid in selected_Acid:
            for condition in ['A','B','C','D']:
                color = cmap(norm(conversion[index_map[condition],count]))
                track1.rect(start=count, end=count+1, r_lim=position_map[condition], color=color)
            condition = s_condition[acid]
            color = cmap(norm(conversion[index_map[condition],count]))
            track1.rect(start=count, end=count+1, r_lim=position_map[condition], ec="black", lw=0.5, color=color)
    ...
    colors = [cmap(norm(inhibition[0,count])) if val != 10 else "grey" for count,val in enumerate(y_labels) if count in selected_index]
    ...
    scatter_track.scatter(scaledup_index, scaledup_label, vmin=0, vmax=len(labels)-1, c=colors, marker='o', s=30, alpha=0.5)
...
#plt.show()
#exit()
plt.savefig('circos_SI.png',dpi=600) 
